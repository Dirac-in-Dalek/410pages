...
- Supabase 세션
  ...
  - 기본 네임스페이스:
    ...
    - 필요 시 `sb-<project-ref>-auth-token-user`도 함께 정리 가능해야 함
...
- 로그인 옵션 변경만으로 client를 재생성하지 않는다
- adapter 내부가 매 호출마다 대상 storage를 선택해야 한다

### Adapter Contract

안전한 구현을 위해 storage adapter는 "생성 시점에 storage를 고정 캡처"하면 안 된다. `getItem`, `setItem`, `removeItem`가 호출될 때마다 현재 정책을 읽고 storage를 고르는 구조여야 한다.

권장 형태:

- `getItem(key)` 호출 시 현재 `autoLoginEnabled`를 읽고 그에 맞는 storage에서 조회
- `setItem(key, value)` 호출 시 현재 `autoLoginEnabled`를 읽고 그에 맞는 storage에 저장
- `removeItem(key)` 호출 시 현재 정책 storage에서 제거하되, logout/cleanup 루틴에서는 양쪽 storage를 모두 정리

이 방식을 쓰면 로그인 직전 플래그 변경과 저장소 선택이 어긋나는 문제를 줄일 수 있고, 이미 생성된 Supabase client가 잘못된 storage reference를 계속 들고 있는 문제도 피할 수 있다.
...
2. 해당 플래그를 참조하는 auth storage adapter를 준비한다.
3. 그 adapter로 Supabase client를 lazy init 또는 init 함수 경유로 초기화한다.
...
- `autoLoginEnabled`를 읽기 전에 module top-level에서 eager client 생성 금지
- `window`/storage 접근은 브라우저 진입 이후로 미루기
- 초기 세션 복원보다 client 준비가 먼저 완료되어야 함
...
- `자동로그인` 플래그 저장이 `signInWithPassword()`보다 먼저 실행되어야 한다
- 그래야 첫 세션 write부터 올바른 storage로 들어간다
...
3. 세션 저장소(`localStorage`, `sessionStorage`)에 남아 있을 수 있는 auth 키를 양쪽 모두 정리
4. `rememberedLoginEmail`은 그대로 유지

이렇게 하면 로그아웃 직후에는 자동 복원이 막히지만, 이메일은 계속 남는다.

정리 대상은 최소한 다음 범위를 포함한다.

- 현재 Supabase client의 `storageKey`
- `storageKey + '-user'`
- 필요 시 `sb-<project-ref>-auth-token*` 네임스페이스 전반

`signOut()`만으로 정리를 끝냈다고 가정하지 않는다.

### Policy Reconciliation On Init

초기화 시점에는 현재 정책과 다른 저장소에 남아 있는 세션을 검사하고 정리해야 한다.

예시:

- `autoLoginEnabled = false`인데 `localStorage`에 이전 auth 세션이 남아 있음
- `autoLoginEnabled = true`인데 `sessionStorage`에 이전 탭 세션이 남아 있음

처리 원칙:

- 현재 정책 storage만 신뢰한다
- 다른 storage에 있는 stale 세션은 제거하거나 최소한 무시한다
- 가능하면 초기화 직후 cleanup을 수행해 이후 복원 경로를 단순화한다

이 단계가 없으면 과거 버전에서 남긴 세션이나 정책 변경 직전의 잔여 토큰 때문에 의도와 다른 자동로그인이 발생할 수 있다.
