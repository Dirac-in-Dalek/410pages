create or replace function public.get_or_create_author(requested_name text)
returns jsonb
language plpgsql
security invoker
set search_path = ''
...
begin
  if current_user_id is null or normalized_title = '' then
    raise exception 'Invalid book title' using errcode = '22023';
  end if;
...
revoke all on function public.get_or_create_author(text) from public, anon;
revoke all on function public.get_or_create_book(uuid, text) from public, anon;
grant execute on function public.get_or_create_author(text) to authenticated;
grant execute on function public.get_or_create_book(uuid, text) to authenticated;
