...
describe('Citation typography', () => {
  ...
  it('starts the archive editor at a fixed empty height before typing', () => {
    ...
    const editor = screen.getByPlaceholderText('문장, 인용문 또는 단어를 입력하세요');
    expect((editor as HTMLTextAreaElement).style.height).toBe('40px');
  });
  ...
  it('closes the standalone note input when cancel is pressed with existing memos', async () => {
    ...
    await user.type(screen.getByPlaceholderText('이 인용문에 메모 추가…'), 'Draft memo');
  ...
  it('saves both quote edits and the memo input together', async () => {
    ...
    await user.type(screen.getByPlaceholderText('이 인용문에 메모 추가…'), 'Fresh memo');
