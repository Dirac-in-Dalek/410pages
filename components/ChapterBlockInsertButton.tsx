  ...
  if (!isEditing) {
    return (
      <button
        type="button"
        aria-label={label}
        className="relative flex h-full min-h-5 w-full items-center text-[var(--text-muted)]"
        onClick={onOpen}
        disabled={disabled}
      >
        <span className="h-px flex-1 bg-[var(--border-main)]" />
        <span className="inline-flex absolute left-1/2 -translate-x-1/2 h-5 whitespace-nowrap opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 [@media(hover:none)]:opacity-100 items-center gap-1 rounded-full border border-[var(--border-main)] bg-[var(--bg-card)] px-2 text-[0.68rem] font-medium text-[var(--text-secondary)] shadow-[0_1px_2px_rgba(28,22,16,0.04)] transition-colors hover:border-[var(--accent-border)] hover:bg-[var(--accent-soft)] hover:text-[var(--accent)]">
          <Plus size={12} />
          챕터 추가
        </span>
        <span className="h-px flex-1 bg-[var(--border-main)]" />
